...
parser.add_option('-o', '--out'        ,    dest='output'             , help='output directory'                                   , default='/data_CMS/cms/ortona/test_submit_to_tier3/')
...
parser.add_option('-a', '--skiprunning',    dest='skipRunning'        , help='skip previous production running jobs (hardcoded)'  , default=0, type=int)
...
jobsDir=cmsswBase+'/src/tier3/JobLauncher_%s_%dEvents_%dSkipped_%s'%(opt.nameTag, opt.maxevents, opt.offset, timeTag)
...
if not outFullPath.endswith('/'):
    outFullPath += '/'
outFullPath += 'miniAOD_%s_%dEvents_%dSkipped_%s/'%(opt.nameTag, opt.maxevents, opt.offset, timeTag)
...
for n in xrange(0,opt.njobs):
    ...
    outShortFN = 'output_%d.root' % n
    outFullLFN += outShortFN

    replacements = {
                    'XXX_MAXEVENTS_XXX':str(EvPerJob),
                    'XXX_SKIPEVENTS_XXX':str(n*EvPerJob + opt.offset),
                    'XXX_SAMPLEFILENAME_XXX':opt.samplefile+ outShortFN,
    ...
    scriptFile.write('export SCRAM_ARCH=slc6_amd64_gcc481\n')
    ...
    #temporary fix: submit only finished file
    skipJob=False
    runningJobs = [32,38,40,41,42,43,47,53,58,59,60,67,131,132,133,134,135,136,137,138,142,145,146]
    for k in range(len(runningJobs)):
        if runningJobs[k]==n :
            skipJob=True
            break
    if skipJob and opt.skipRunning == 1 :
        print "skipping (running) job %d " % n
    elif not skipJob and opt.skipRunning == 2:
        print "skipping (already done) job %d" % n
    else :     
        if opt.queue=='':
            os.system('%s/runJob_%d.sh'%(jobsDir.jobSeed))
        else:
            os.system("/opt/exp_soft/cms/t3/t3submit -q cms \'%s/runJob_%d.sh\'"%(jobsDir,n))
            #print "would have submitted job %d" % n
